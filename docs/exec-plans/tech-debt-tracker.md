# Tech Debt Tracker

## Open

- Add Playwright smoke tests for editor selection, mutation, and export flows.
- Expand DSL validation from lightweight runtime checks to schema-level validation.
- Improve exported React formatting.
- Strengthen contrast checks for section-level backgrounds and primary buttons.
- Replace mock AI with a real adapter and environment-based model config.
- Add cloud persistence and authenticated project history.
- Add a release checklist once hosted preview deployment exists.

## Closed

- Initial repository harness, DSL, renderer, editor, quality score, exporter, and docs sync are complete.
- Local browser persistence for the active project DSL is complete.
- GitHub CI, issue templates, PR template, and repository operations docs are complete.
- Handoff context, cold-start docs, and repository verification scripts are complete.
- Portable project JSON import/export for cross-device DSL transfer is complete.
- Chinese customer-facing MVP flow and visible button feedback are complete.
- Newly added and duplicated modules are selected immediately after mutation.
